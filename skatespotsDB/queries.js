...
module.exports = {
    ...
    update(tableName, id, table_object){
        console.log("queries.update and the id passed to it: ", id);
        return database(tableName).update(table_object).where('id', id).returning('*')
            .then(record => record[0]); 
